...
var nodeAccumulator = (function () {
    var prototype = {
        ...
        '.': function () {
            ...
            ++this.index;
            this.lastIndex = this.index;
            var cp1 = this.expr.charAt(this.index);
            if (cp1 === '.') {
                this.exprList.push(types.RECURSIVE_DESCENT);
                ++this.index;
                this.lastIndex = this.index;
            } else if (this.index === this.length) {
                throw new Error('Incomplete path.');
            }
            if (this.expr.charAt(this.index) === '.') {
        ...
        '[': function () {
            ...
            ++this.index;
            this.lastIndex = this.index;
            this.index = findMatchingSquareBracket(this.expr, this.index);
            var subExprInside = this.expr.substring(this.lastIndex, this.index);
            var scriptResult = subscriptToNode(subExprInside);
            this.pushNode(scriptResult);
            return true;
        },
        '*': function () {
            if (this.lastIndex === this.index) {
                this.pushNode(types.WILDCARD);
                return true;
            } else {
                return false;
            }
        ...
        '$': function () {
            if ((this.index === 0) && (this.expr.charAt(1) === '.')) {
                this.pushNode(types.ROOT);
                return true;
            } else {
                return false;
            }
