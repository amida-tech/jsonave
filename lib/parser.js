...
exports.normalize = function (expr) {
    ...
    while (index < length) {
        ...
        if (c === '[') {
            ...
            ++index;
            lastIndex = index;
            index = findMatchingSquareBracket(expr, index);
            if (index < length) {
        ...
        if (c === '^') {
            if (lastIndex !== index) {
                var subExprx = expr.substring(lastIndex, index);
                updateNodesWithProperty(subExprx, exprList);
            }
